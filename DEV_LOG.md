...
#### ✅ 미니게임을 실서비스 `/mini-game` 페이지로 이전

`/demo/mini-game`에 결과 저장 기능을 붙였다가, 다른 실서비스 페이지(`/stocks`, `/account-book`)와 패턴을 맞추기 위해 로그인 필수 실서비스 `/mini-game`을 새로 만들고 데모는 순수 로컬 버전으로 되돌림.

**신규 — `apps/web/src/app/mini-game/`**
- `layout.tsx` — `../dashboard/layout` 재사용 (auth guard)
- `page.tsx` — 데모에서 포팅, `useSession` 체크 없이 항상 저장(로그인 필수 구조라 불필요), 직업 선택 모달 초기값 `true`(진짜 게임 시작 플로우)
- `types.ts`, `data/*`, `lib/gameLogic.ts`, `components/*` — 데모에서 동일 복사 (게임 로직 자체는 데모 전용 요소 없어 그대로 재사용)
- 사이드바 nav에 `/mini-game` 링크가 이미 있어 별도 라우팅 작업 불필요

**`/demo/mini-game` 원복**
- `useSession`/`api` 관련 코드, `saveResult`, `GameHistoryModal` 연동 전부 제거 — 다른 데모 페이지들과 동일하게 API 호출 없는 순수 로컬 버전으로 복원
- 안 쓰는 `GameHistoryModal.tsx` 데모 폴더에서 삭제 (실서비스 폴더에만 유지)

#### 🐛 버그 픽스 — 헤더 레이아웃이 현금 숫자 길이에 따라 흔들림

사람이 게임을 이겨서 보유 현금이 "1억 4,044만 8,000원"처럼 길어지자, 헤더의 "쥐경주 탈출률" 바 + 버튼 영역이 `flex-wrap`으로 인해 위치가 들쭉날쭉해지는 문제 발견.
→ `GameHeader.tsx`를 1행(로고/타이틀 + 통계 박스), 2행(탈출률 바 + 버튼)으로 구조 자체를 고정 — 컨텐츠 폭과 무관하게 항상 2행 레이아웃 유지. `/mini-game`·`/demo/mini-game` 양쪽 반영.

#### 🐛 버그 픽스 — "자녀 출산" 카드가 실제로 자녀 수를 증가시키지 않음

사람이 "자녀가 계속 0명에서 1명으로만 나온다" 제보. 확인해보니 `CardModal.tsx`의 baby 카드 "확인" 버튼이 `onClose`만 호출하고 `childrenCount`를 실제로 갱신하는 로직이 아예 없었음 — 화면의 "0명 → 1명"은 실제 상태가 아니라 매번 동일하게 계산되는 미리보기 텍스트였을 뿐.
→ `handleAcceptBaby` 핸들러 추가 (최대 3명 캡, 활동 로그 기록) — `page.tsx`에 구현 후 `CardModal`에 `onAcceptBaby` prop으로 연결. 양쪽 폴더 반영.

#### 🐛 버그 픽스 — 보드 중앙 카드 높이가 칸 설명 길이에 따라 흔들림

칸마다 설명 텍스트 길이가 달라 1줄/2줄로 줄바꿈되면서 중앙 카드(현재 위치+주사위+버튼) 전체 높이가 흔들리던 문제.
→ `BoardView.tsx` 설명 `<p>`에 `min-h-[2rem]`(2줄 분량) 고정해 텍스트 길이와 무관하게 높이 유지.

**검증**: 타입체크 통과, lint는 기존에 있던 경고(다른 핸들러의 `let`/미사용 import)만 양쪽에 동일하게 남아있음 — 이번 변경으로 인한 신규 이슈 없음. 실제 동작 확인은 사람이 직접 진행.
