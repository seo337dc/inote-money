...
- **커밋·PR·push는 사용자가 명시적으로 요청할 때만.** 기능 구현·lint/build 통과·브라우저 검증까지
  끝났다고 해서 자동으로 이어가지 않는다 — "구현해줘"는 커밋 요청이 아니다. 구현이 끝나면 결과만
  보고하고 멈춘다.
...
#### 🔥 모바일 앱 (현재 최우선, 2026-08-19 확정)

> 상세 체크리스트·진행 방식은 [`apps/app/CLAUDE.md`](apps/app/CLAUDE.md) 참고. 여기는 우선순위 표시만.

웹 서비스 기본 기능은 이미 충분히 갖춰져 있으므로, WebView로 감싸는 순간 "기본 기능 되는 앱"이 나온다는 전제로 아래 순서를 최우선으로 진행:

- [ ] 단계 1 — 환경 준비 (Expo Go, `create-expo-app`)
- [ ] 단계 2 — **WebView로 URL 붙이기** ← 여기서 1차 완성
- [ ] 단계 3 — EAS Build로 실제 APK 뽑아 설치
- [ ] 단계 4 — 로그인 플로우 (쿠키 교환 방식)
- [ ] 단계 5 — AI 문자 인식 자동 가계부 (Android 전용)
- [ ] 단계 6 — 배포 여부 결정

이 항목이 끝나기 전까지 아래 🔴🟡🟢🔵 항목들(대부분 이미 완료됨)보다 우선한다.
