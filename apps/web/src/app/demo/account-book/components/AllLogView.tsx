...
import { TriangleAlert, Plus, Trash2, Pencil, Check, X } from "lucide-react";
...
export default function AllLogView({ allExpenses, onAddExpense, onDeleteExpense, onEditExpense }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {hasMore ? (
        <div ref={sentinelRef} className="flex items-center justify-center py-6 text-sm text-gray-400 dark:text-gray-500">
          불러오는 중...
        </div>
      ) : (
        <p className="text-center text-xs text-gray-400 dark:text-gray-500 py-6">모든 내역을 불러왔습니다</p>
      )}
    </div>
  );
}
